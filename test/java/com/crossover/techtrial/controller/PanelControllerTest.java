...
public class PanelControllerTest {
  ...
  public void testPanelShouldSaveHourlyElectricity throws Exception {
    HttpEntity<Object> hourlyElectricity = getHttpEntity(
        "{\"generatedElectricity\": \"200\", \"readingAt\": \"2018-01-31 09:00:00\" }");
    ResponseEntity<HourlyElectricity> response = template.postForEntity(
        "/api/panels/2/hourly", hourlyElectricity, HourlyElectricity.class);
    Assert.assertEquals(200,response.getStatusCode().value());
  }

  @Test
  public void testPanelGetHourlyElectricity throws Exception {
 //   HttpEntity<Object> hourlyElectricity = getHttpEntity(
   //     "{\"generatedElectricity\": \"200\", \"readingAt\": \"2018-01-31 09:00:00\" }");
		String expected = "{\"generatedElectricity\": \"200\", \"readingAt\": \"2018-01-31 09:00:00\" }";

    ResponseEntity<Page> response = template.getForEntity(
        "/api/panels/2/hourly", Page<HourlyElectricity>.class);
   // assertThat(response.getStatusCode(), equalTo(HttpStatus.OK));
    
	Assert.assertEquals(expected, response.getBody(), true);
}
